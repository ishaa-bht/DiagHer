...
class DiagnosisPredictor:
    """Enhanced diagnosis prediction class with detailed response"""
    ...
    def get_relevant_symptoms(self, input_data: Dict, condition: str) -> List[str]:
        """Get symptoms that are relevant for a specific condition"""
        try:
            symptoms = input_data.get('symptoms', {})
            
            # Get active symptoms (those with value > 0)
            active_symptoms = [symptom for symptom, value in symptoms.items() if value > 0]
            
            # If model has feature importance, we could use that to rank symptoms
            # For now, return all active symptoms
            # In a more sophisticated implementation, you might have condition-specific symptom mappings
            
            return active_symptoms
            
        except Exception as e:
            logging.error(f"Error getting relevant symptoms: {str(e)}")
            return []
    
    def predict_detailed(self, input_data: Dict) -> Dict:
        """Make detailed diagnosis prediction with top 3 conditions"""
        try:
            if self.model is None:
                logging.error("Model not loaded")
                return {
            ...
            # Format predictions with detailed information
            formatted_predictions = []
            for condition, probability in top_3_predictions:
                confidence_percentage = round(probability * 100, 2)
                confidence_level = self.get_confidence_level(confidence_percentage)
                relevant_symptoms = self.get_relevant_symptoms(input_data, condition)
            ...
            symptoms = input_data.get('symptoms', {})
            total_symptoms = sum(1 for value in symptoms.values() if value > 0)
            ...
            logging.info(f"Generated detailed prediction for {len(formatted_predictions)} conditions")
            return response
            
        except Exception as e:
            logging.error(f"Error making detailed prediction: {str(e)}")
            return {
                "status": "error",
    ...
    def predict(self, input_data: Dict) -> List[Tuple[str, float]]:
        """Make diagnosis prediction (original method for backward compatibility)"""
        try:
            if self.model is None:
                logging.error("Model not loaded")
                return []
...
if __name__ == "__main__":
    ...
    # Test prediction with detailed response
    test_data = {
        'age': 45,
        'gender': 'Female',
        'symptoms': {
    ...
    if predictor.is_model_loaded():
        ...
        detailed_response = predictor.predict_detailed(test_data)
        print(f"\nDetailed Response:")
